...
moi = 'nRv_f4_cap2_t20_etaf'; % model of interest

addpath(genpath('C:\Users\SN\Google Drive\z.functions\models'));
outdir = 'C:\Users\SN\Google Drive\p1.UG2_fMRI\180422_eta.8';

% data generation
load ('Modelfit_NC.mat')% to get params (for data generation)
m = 5; % model number in the date file
nP = length(OFFER);
param_tru = param(:, :, m);
Xtru = X(:, :, m);
nx = sum(freeID(m, :));
...
inModel = Model{m};
save(['recover_', moi, '_NC','.mat'], 'c', 'moi', 'nx', 'param_tru', 'param_est', 'Xtru', 'Xest', 'Rsim', 'R', 'P', 'inModel')
